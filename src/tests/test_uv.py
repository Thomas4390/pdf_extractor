...
Usage:
    python -m src.tests.test_uv [pdf_path] [options]
...
Examples:
    python -m src.tests.test_uv                          # Default PDF and mode
    python -m src.tests.test_uv --mode hybrid            # Test hybrid mode
    python -m src.tests.test_uv --mode vision --force    # Force vision extraction
    python -m src.tests.test_uv path/to/file.pdf --mode pdf_native
"""
...
async def test_extraction(
    ...
    if pdf_path is None:
        ...
        if not pdf_files:
            ...
            return None
        pdf_path = pdf_files[-2]  # Use second to last PDF
    else:
        pdf_path = Path(pdf_path)

    # Get original model configuration
    original_config = get_model_config("UV")
    ...
    try:
        ...
        if mode or model:
            register_model("UV", original_config)
