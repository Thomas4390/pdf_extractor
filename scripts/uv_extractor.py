...
class RemunerationReportExtractor:
    ...
    def extract_report_date(self, text: str) -> Optional[str]:
        ...
        """
        pattern = r'RAPPORT\s+DE\s+RÉMUNÉRATION\s+DU\s+(\d{4}-\d{2}-\d{2})'
    ...
    def extract_advisor_number(self, text: str) -> Optional[str]:
        ...
        """
        pattern = r'Numéro\s+du\s+conseiller\s*:?\s*(\d+)'
        match = re.search(pattern, text, re.IGNORECASE)
    ...
    def clean_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_clean['Résultat'] = df_clean['Résultat'].apply(self.clean_amount)
        df_clean['Taux de Boni'] = df_clean['Taux de Boni'].apply(self.clean_percentage)
        df_clean['Rémunération'] = df_clean['Rémunération'].apply(self.clean_amount)

        # Clean text columns (remove line breaks and excess spaces)
        text_columns = ['Contrat', 'Assuré(s)', 'Protection', 'Type']
    ...
    def extract_all(self) -> Dict:
        ...
        try:
            with pdfplumber.open(self.pdf_path) as pdf:
                ...
                if raw_df.empty:
                    print("❌ ERROR: No data could be extracted from the PDF")
                    return result
    ...
    def display_results(self, data: Dict):
        ...
        if data['activites'] is not None and not data['activites'].empty:
            ...
            print(f"\nTOTAL REMUNERATION: {df['Rémunération'].sum():,.2f} $")
        else:
...
if __name__ == "__main__":
    ...
    if data['activites'] is not None:
        ...
        print(f"Total remuneration: {df['Rémunération'].sum():,.2f} $")
        print(f"Average remuneration per protection: {df['Rémunération'].mean():,.2f} $")
        print("=" * 100)